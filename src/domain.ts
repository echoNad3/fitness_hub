...
export function resultGuidance(streak: WorkoutResultStreak) {
  if (!streak.result) {
    return "No previous attempt. Choose today's weight."
  }

  const multiplier = streak.count >= 2 ? ` x${streak.count}` : ''
  const attemptWord = streak.count >= 2 ? 'attempts' : 'attempt'

  return streak.result === 'success'
    ? `Last ${attemptWord}: done${multiplier}. Increase today.`
    : `Last ${attemptWord}: failed${multiplier}. Repeat today.`
}
