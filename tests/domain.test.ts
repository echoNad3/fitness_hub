...
test('result guidance adds a multiplier and plural only from the second matching result', () => {
  assert.equal(resultGuidance({ result: undefined, count: 0 }), "No previous attempt. Choose today's weight.")
  assert.equal(resultGuidance({ result: 'success', count: 1 }), 'Last attempt: done. Increase today.')
  assert.equal(resultGuidance({ result: 'success', count: 3 }), 'Last attempts: done x3. Increase today.')
  assert.equal(resultGuidance({ result: 'failure', count: 1 }), 'Last attempt: failed. Repeat today.')
  assert.equal(resultGuidance({ result: 'failure', count: 2 }), 'Last attempts: failed x2. Repeat today.')
})
