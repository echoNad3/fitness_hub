...
test('a legacy backup with valid sessions and no templates is accepted', () => {
  ...
  assert.equal(isValidBackup({ sessions: [], variantOverrides: { exercise: { sets: 1000 } } }), false)
  assert.equal(isValidBackup({ sessions: [], variantOverrides: { exercise: { note: 'x'.repeat(241) } } }), false)
...
test('new optional fields must be well-typed when present', () => {
  ...
  assert.equal(isValidSessions([{ ...session, endedEarly: true }]), true)
  assert.equal(isValidSessions([{ ...session, finishedAt: 'noon' }]), false)
  assert.equal(isValidSessions([{ ...session, endedEarly: false }]), false)
  assert.equal(isValidSessions([{ ...session, finishedAt: undefined, endedEarly: true }]), false)
})
...
test('session entries reject invalid weights and result values', () => {
  ...
  assert.equal(
    isValidSessions([{ ...validSession, groupEntries: { exercise: { activeVariantId: 'exercise', entries: { exercise: { weight: 20, perHand: 'yes' } } } } }]),
    false,
  )
